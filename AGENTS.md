...
- Help coding agents ship safe changes quickly in the `telli-dialog` monorepo (TypeScript + Turborepo).
...
- User-facing apps are Next.js: `apps/dialog` (main product) and `apps/admin` (configuration UI).
- Backend is Fastify: `apps/api` (`src/app.ts`, `src/handlers.ts`) exposing `/v1/*` and `/docs`.
- Two separate Postgres databases:
  - Dialog/Admin DB in `packages/shared/src/db` (migrations + seed).
  - API DB in `packages/api-database/src` (schema in `schema.ts`, seed in `seed.ts`).
- AI flow: UI/apps call shared services, then API/LLM integration goes through `@telli/ai-core`; `packages/shared/src/knotenpunkt/index.ts` calls `${env.apiUrl}/v1/models`.
...
- Reuse UI from `@telli/ui` before creating local components (`packages/ui/README.md`).

## Developer workflows (root)

- Install/tooling: `nvm use && corepack enable && corepack prepare && pnpm i`.
- Run all apps: `pnpm dev` (Turbo). Run one app: `pnpm dev:dialog`, `pnpm dev:admin`, `pnpm dev:api`.
...
- Internationalize UI text (dialog uses `next-intl`; see `apps/dialog/src/components/**` for `useTranslations(...)`).
...
- Auth: Keycloak (local realm in `devops/docker/keycloak/telli-local-realm.json`).
