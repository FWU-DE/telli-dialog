...
describe('calculator tool', () => {
  ...
  it('returns EXPRESSION_TIMEOUT, sends SIGKILL, and releases an active slot when child hangs', async () => {
    ...
    try {
      ...
      const second = buildCalculatorTool({
        spawnProcess: vi.fn(() => next as never),
        maxConcurrent: 1,
        maxQueue: 0,
      }).handler({ expression: '1' });
      next.stdout.end('{"ok":true,"result":"1"}');
      next.emit('close', 0);
  ...
  it('returns PROTOCOL_ERROR and releases slot on synchronous spawn throw', async () => {
    ...
    const tool = buildCalculatorTool({ spawnProcess, maxConcurrent: 1, maxQueue: 0 });
