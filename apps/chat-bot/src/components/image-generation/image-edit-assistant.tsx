...
import { imageEditAssistantAction } from '@/app/(authed)/(chat-bot)/image-generation/image-edit-assistant-actions';
import { type AssistantMessage } from '@/app/(authed)/(chat-bot)/image-generation/image-edit-assistant-service';
...
interface ImageEditAssistantProps {
  open: boolean;
...
export function ImageEditAssistant({
  open,
  onOpenChange,
  onPromptGenerated,
  originalPrompt,
  imageUrl,
}: ImageEditAssistantProps) {
  ...
  async function sendMessage(text: string) {
    ...
    const result = await imageEditAssistantAction(next, { originalPrompt, imageUrl });
    if (result.success) {
      ...
      >
        <SheetHeader className="px-5 pt-5 pb-4 border-b shrink-0">
          <SheetTitle className="flex items-center gap-2">
            ...
            {t('edit-assistant-title')}
          </SheetTitle>
          <SheetDescription>{t('edit-assistant-description')}</SheetDescription>
        </SheetHeader>
