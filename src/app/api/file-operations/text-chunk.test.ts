...
describe('chunkText', () => {
  it('chunks text by sentences with overlap', async () => {
    ...
    expect(chunks[0]).toContain('Here is the second sentence.');
    expect(chunks[1]).toContain('Here is the second sentence.');
    expect(chunks[1]).toContain('This is the third sentence.');
  });
  ...
  it('ensures sentence overlap at chunk boundaries', async () => {
    ...
    expect(chunks[1]).toContain('Sentence two with a lot of words.');
    expect(chunks[1]).toContain('Sentence three with a lot of words.');
    expect(chunks[1]).toContain('Sentence four with a lot of words.');
  });
  ...
  it('handles 5 short sentences and one very long sentence (>200 words)', async () => {
    ...
    expect(chunks[chunks.length - 1]).toContain('going, with more and more words');
  });
}); 