...
import { formatBytes, getFileExtension, getFileNameAndFileExtention, hexToRGBA } from '@/utils/files/generic';
...
export default function FilesTable({ files, onDeleteFile, additionalFiles }: FilesTableProps) {
  if (files.length < 1 && additionalFiles.size < 1) return null;

  function handleOpenDocument(fileId: string) {
    window.open(`/file_display/${fileId}`, '_blank');
  }
  console.log(additionalFiles);
  const mergedFiles = [
    ...files.map((f) => ({ id: f.id, size: f.size, fileName: f.name, status: 'processed' as const })),
      ...
      <tbody>
        ...
        const [fileStem, extention ] = getFileNameAndFileExtention(fileName)
          ...
          return (
            <tr
              key={id}
              className="border-b-[1px] last:border-b-0 border-[#D9D9D9]"
            >
